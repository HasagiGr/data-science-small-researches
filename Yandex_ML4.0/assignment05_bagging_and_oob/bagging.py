...
class SimplifiedBaggingRegressor:
    ...
    def _get_oob_predictions_from_every_model(self):
        ...
        for idx, model in enumerate(self.models_list):
            oob_indices = set(range(len(self.data))) - set(self.indices_list[idx])
            for oob_idx in oob_indices:
                prediction = model.predict(self.data[oob_idx].reshape(1, -1))
                list_of_predictions_lists[oob_idx].append(prediction)
    ...
    def _get_averaged_oob_predictions(self):
        ...
        # Your Code Here
        self.oob_predictions = np.array([np.mean(predictions) if predictions else np.nan for predictions in self.list_of_predictions_lists])
    ...
    def OOB_score(self):
        ...
        return np.mean((valid_targets-valid_predictions)**2) 